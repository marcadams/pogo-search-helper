...
### Added
- **PvP IV Rank Checker tab** - select a Pokemon from a searchable dropdown with sprites, input IVs via sliders, see rank out of 4,096 combos with color-coded badge, stat product percentage, best level, and CP
- **110 Pokemon** in PvP database with GO-specific base stats and dex numbers
- **PvP explainer** - collapsible section explaining the CP formula, why low Attack IVs are better in capped leagues, stat product concept, and when it doesn't apply
- **Searchable dropdown selector** with sprites (replaces tappable pill list)
- **Type Chart tab** - interactive 18x18 effectiveness matrix with heatmap, tap-to-inspect detail view, color-coded legend
- **Collapsible category sections** - clickable headers with count badges and "selected" indicators
- **Collapse All / Expand All** buttons
- **Inline tutorial panel** - dismissable AND/OR/NOT help with examples, persists in localStorage
- **SEO meta tags** - Open Graph, Twitter Card large image, canonical URL, keywords, social preview image
- **Sticky tab navigation** - pinned to top on scroll
- **Scroll-to-content on tab switch** - using non-sticky anchor element for reliable scrolling
- **Hero mini Pokeball** on mobile - compact inline SVG replacing the full graphic
- **Rebranded** - "Trainer Toolkit" with broader subtitle
- **Deploy workflow gates on tests** - npm test runs before npm run build
- **New test suites** - pvp.test.ts (15 tests), typeChart.test.ts (12 tests)

### Changed
- **Sections collapsed by default on mobile** (< 768px)
- **Hero graphic hidden on mobile** - replaced with 36px inline Pokeball
- **Tab bar is sticky** with position sticky
- **Page title** updated for SEO breadth
